...
export class GameEngine {
  ...
  private getTileColor(tileType: string): string {
    switch (tileType) {
      ...
      case 'sand': return '#f4e4bc';
      case 'dirt': return '#8b4513';
      case 'wood': return '#8b4513';
      case 'wall': return '#444444';
      case 'floor': return '#cccccc';
  ...
  private renderTiles() {
    ...
    for (let y = this.renderBounds.startY; y < this.renderBounds.endY && y < actualMapHeight; y++) {
      for (let x = this.renderBounds.startX; x < this.renderBounds.endX && x < actualMapWidth; x++) {
        if (!this.gameState.currentMap.tiles[y] || !this.gameState.currentMap.tiles[y][x]) {
          continue;
        }
  ...
  private renderPlayer() {
    ...
    // Direction indicator
    this.ctx.fillStyle = '#ffffff';
    const dirX = screenX + (this.gameState.player.direction === 'right' ? 8 : this.gameState.player.direction === 'left' ? -8 : 0);
    const dirY = screenY + (this.gameState.player.direction === 'down' ? 8 : this.gameState.player.direction === 'up' ? -8 : 0);
  ...
  private renderNPCs() {
    this.gameState.currentMap.npcs.forEach(npc => {
      ...
      if (screenX < -32 || screenX > this.canvas.width || 
          screenY < -32 || screenY > this.canvas.height) return;
  ...
  private renderNearbyEntities() {
    // Render only entities near the player for performance
    const playerPos = this.gameState.player.position;
  ...
  private renderUI() {
    ...
    this.ctx.fillStyle = 'rgba(0, 0, 0, 0.7)';
    this.ctx.fillRect(10, 10, healthBarWidth + 10, healthBarHeight + 10);
    
    this.ctx.fillStyle = '#333333';
    this.ctx.fillRect(15, 15, healthBarWidth, healthBarHeight);
    
    this.ctx.fillStyle = '#ff4444';
    this.ctx.fillRect(15, 15, healthBarWidth * healthPercent, healthBarHeight);
  ...
  private renderSimpleUI() {
    ...
    this.ctx.fillStyle = '#ff4444';
    this.ctx.fillRect(15, 15, 140 * healthPercent, 15);
    
    this.ctx.fillStyle = '#ffffff';
    this.ctx.font = '12px Arial';
    this.ctx.textAlign = 'left';
    this.ctx.fillText(`HP: ${this.gameState.player.health}/${this.gameState.player.maxHealth}`, 20, 26);
  }
  ...
  private enterBuilding(buildingId: string) {
    ...
    this.gameState.previousMap = this.gameState.currentMap;
    this.gameState.previousPosition = { ...this.gameState.player.position };
    
    let interiorMap = this.loadedMaps[building.interior.id];
    if (!interiorMap) {
      const createMapFn = maps[building.interior.id as keyof typeof maps];
      if (createMapFn) {
        interiorMap = createMapFn();
        this.loadedMaps[building.interior.id] = interiorMap;
      } else {
        return;
      }
    }
    ...
    this.gameState.player.position = { ...building.interior.spawnPosition };
  ...
  private exitBuilding() {
    if (!this.gameState.previousMap || !this.gameState.previousPosition) return;
    
    this.gameState.currentMap = this.gameState.previousMap;
    this.gameState.player.position = { ...this.gameState.previousPosition };
    
    this.gameState.previousMap = undefined;
    this.gameState.previousPosition = undefined;
